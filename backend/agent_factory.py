...
# Respect privacy in local mode - disable tracing when telemetry is disabled
from .utils.globals import DISABLE_TELEMETRY
...
def create_agent(**kwargs) -> Agent:
    ...
    # Allow config override (works in both local and non-local mode)
    if config:
