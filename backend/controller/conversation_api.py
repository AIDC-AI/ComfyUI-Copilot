...
from ..service.debug_agent import debug_workflow_errors, debug_workflow_errors_single
...
from ..service.all_in_one_agent import all_in_one_invoke
...
@server.PromptServer.instance.routes.post("/api/chat-single/invoke")
async def invoke_chat_single(request):
    log.info("Received invoke_chat_single request")
    ...
    historical_messages = req_json.get('messages', [])
    workflow_checkpoint_id = req_json.get('workflow_checkpoint_id')
    workflow_data = req_json.get('workflow_data')  # Support debug-style payloads
    ...
    config = {
        ...
        "openai_api_key": request.headers.get('Openai-Api-Key'),
        "openai_base_url": request.headers.get('Openai-Base-Url'),
        # Optional Workflow LLM settings
        "workflow_llm_api_key": request.headers.get('Workflow-LLM-Api-Key'),
        "workflow_llm_base_url": request.headers.get('Workflow-LLM-Base-Url'),
        "workflow_llm_model": request.headers.get('Workflow-LLM-Model'),
        "model_select": next((x['data'][0] for x in ext if x['type'] == 'model_select' and x.get('data')), None) if ext else None
    }

    # Set request context (isolation per session)
    set_request_context(session_id, workflow_checkpoint_id, config)

    # If workflow_data provided (debug scenario), save it so tools like run_workflow/get_current_workflow can access
    try:
        if workflow_data:
            version_id = save_workflow_data(
                ...
                attributes={
                    "checkpoint_type": "debug_start_single",
                    "description": "Initial workflow save for single-agent invoke",
                    "timestamp": time.time()
                }
            )
            log.info(f"[Single] Saved incoming workflow_data, version ID: {version_id}")
    except Exception as save_err:
        log.error(f"[Single] Failed to save workflow_data: {save_err}")

    processed_images = []
    if images and len(images) > 0:
        log.info(f"Note: Received {len(images)} images in legacy format, but using OpenAI message format instead")

    # Ensure messages exist and start with a user message (required by agents streaming)
    openai_messages = historical_messages if isinstance(historical_messages, list) else []
    needs_user_head = (len(openai_messages) == 0) or (openai_messages[0].get('role') != 'user')
    if needs_user_head:
        # If workflow_data present, default to a debug instruction; otherwise a generic chat starter
        default_user_text = "Validate and debug this ComfyUI workflow." if workflow_data else "Start the conversation."
        openai_messages = [{"role": "user", "content": default_user_text}] + openai_messages
    ...
    try:
        ...
        log.info(f"[Single] config: {config}")

        async for result in all_in_one_invoke(openai_messages, None):
            if isinstance(result, tuple) and len(result) == 2:
                text, ext_with_finished = result
                if text:
                ...
                if ext_with_finished:
                    ext_data = ext_with_finished.get("data")
                    finished = ext_with_finished.get("finished", True)
                    log.info(f"[Single] -- Received ext: {bool(ext_data)}, finished: {finished}")
            else:
                text_chunk = result
                if text_chunk:
                    accumulated_text += text_chunk
                    log.info(f"[Single] -- Received text chunk: '{text_chunk}', total length: {len(accumulated_text)}")
        ...
        log.error(f"Error in invoke_chat_single: {str(e)}")
        error_response = ChatResponse(
            session_id=session_id,
            text=f"I apologize, but an error occurred: {str(e)}",
            finished=True,
...
@server.PromptServer.instance.routes.post("/api/debug-agent-single")
async def invoke_debug_single(request):
    """
    Single-agent Debug endpoint for analyzing ComfyUI workflow errors
    """
    log.info("Received debug-agent-single request")
    ...
    log.info(f"Single Debug agent config: {config}")
    ...
    try:
        ...
        async for result in debug_workflow_errors_single(workflow_data):
            if isinstance(result, tuple) and len(result) == 2:
                text, ext = result
                if text:
                    accumulated_text = text
            ...
            try:
                ...
                if final_response["ext"]:
                    final_response["ext"].append({
                        ...
                        "data": {"checkpoint_id": checkpoint_id, "checkpoint_type": "debug_complete"}
                    })
                else:
                    final_response["ext"] = [{
                        "type": "debug_checkpoint",
                ...
                log.info(f"Single Debug completion checkpoint saved with ID: {checkpoint_id}")
            except Exception as checkpoint_error:
                log.error(f"Failed to save single debug completion checkpoint: {checkpoint_error}")
        
        await response.write(json.dumps(final_response).encode() + b"\n")
        log.info("Single Debug agent processing complete")
    
    except Exception as e:
        log.error(f"Error in single debug agent: {str(e)}")
        import traceback
        traceback.print_exc()
        error_response = ChatResponse(
            session_id=session_id,
            text=f"❌ Single Debug agent error: {str(e)}",
            finished=True,
            type="message",
            format="text",
